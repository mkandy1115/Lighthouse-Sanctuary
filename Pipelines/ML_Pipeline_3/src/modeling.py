...
def train_models(X: pd.DataFrame, y: pd.Series, random_state: int = 42):
    ...
    lg = Pipeline([("prep", prep), ("model", LogisticRegression(max_iter=1000, random_state=random_state))])
    ...
    feat_names = lg.named_steps["prep"].get_feature_names_out()
    coefs = lg.named_steps["model"].coef_[0]
    coef_df = pd.DataFrame({"feature": feat_names, "coefficient": coefs, "odds_ratio": np.exp(coefs)}).sort_values("coefficient", ascending=False)
    return rf, rf_metrics, lg, lg_metrics, coef_df
