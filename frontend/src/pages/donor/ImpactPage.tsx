...
import { getDonorDashboard, type DonorDashboardData } from '@/lib/donor'
...
export default function DonorImpactPage() {
  ...
  const impactMetrics = [
    { label: 'Counseling Sessions Funded', value: dashboard?.summary.counselingSessionsFunded ?? 0, unit: 'sessions', color: 'brand-teal' },
    { label: 'Residents Helped (Estimate)', value: dashboard?.summary.residentsHelpedEstimate ?? 0, unit: 'residents', color: 'brand-bronze' },
    { label: 'Donations Made', value: dashboard?.summary.donationCount ?? 0, unit: 'donations', color: 'brand-teal' },
    { label: 'Recurring Gift Active', value: dashboard?.summary.hasRecurringGift ? 1 : 0, unit: 'flag', color: 'brand-bronze' },
  ]
  const outcomes = [
    { label: 'Program Completion', value: metrics?.aggregates.programCompletionPct ?? 0 },
    { label: 'Stable Housing', value: metrics?.aggregates.stableHousingVisitPct ?? 0 },
    { label: 'Wellbeing Stability', value: metrics?.aggregates.wellbeingPct ?? 0 },
  ]
  ...
  return (
    <div className="animate-fade-in">
        ...
        {impactMetrics.map((m) => (
          <div key={m.label} className="bg-brand-cream rounded-xl border border-brand-border p-5 text-center">
            ...
            <p className="text-xs text-brand-muted font-medium">
              {m.unit === 'flag' ? (m.value > 0 ? 'yes' : 'no') : m.unit}
            </p>
      ...
      <div className="bg-brand-cream rounded-xl border border-brand-border p-6 mb-6">
        <h2 className="font-semibold text-brand-charcoal mb-5">Program Outcomes (Live Data)</h2>
      ...
      <div className="bg-brand-teal-muted border border-brand-teal/20 rounded-xl p-6">
        ...
        <p className="text-brand-teal text-xs font-medium mt-3">— Survivor (name withheld)</p>
      </div>
    </div>
  )
}
